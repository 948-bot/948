"""
XAUUSD AI DERIV BOT
Main Entry Point - Clean Version (No Startup Spam)
"""
import time
import json
import websocket
...
def main():
    ...
    try:
        ...
        if not trading_day:
            print("⚠️ Hari ini akhir pekan (Weekend). Bot OFF.")
            return

        # 3. Tarik Data Pasar M30 & M15
        print("🔄 Menarik data pasar M30 & M15 dari server Deriv...")
        df_m30 = fetch_deriv_candles(granularity=1800, count=60)
        df_m15 = fetch_deriv_candles(granularity=900, count=60)

        if df_m30.empty or df_m15.empty:
            err_txt = "Gagal menarik data candle pasar dari server Deriv (Kosong)."
            print(f"⚠️ {err_txt}")
            send_error_notification(err_txt)
            return

        # Cek harga real-time di log console (Tanpa kirim spam ke Telegram)
        ...
        if action in ["BUY", "SELL"]:
            ...
            print("⏸️ Pasar masih WAIT. Bot siaga memantau (Tanpa notif Telegram).")

        print("🏁 Eksekusi pipeline bot selesai.")

    except Exception as e:
        error_msg = f"Critical Error pada Bot Runner: {str(e)}"
        print(f"🔴 {error_msg}")
